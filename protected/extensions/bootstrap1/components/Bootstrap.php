...
class Bootstrap extends CApplicationComponent
{
    ...
    public function registerCoreCss()
    {
        ...
        //Yii::app()->clientScript->registerCssFile($this->getAssetsUrl().'/css/'.'tipped/tipped.css');
    }
    ...
    protected function registerJS($position = CClientScript::POS_HEAD)
    {
        ...
        $cs->registerScriptFile($this->getAssetsUrl() . '/js/' . $filename,
                $position);
        //$cs->registerScriptFile($this->getAssetsUrl() . '/js/tipped/tipped.js', $position);
    }
            ...
            $options = array())
    {
        ...
        if (isset($config['options']))
            $options = !empty($options) ? CMap::mergeArray($options,
                            $config['options']) : $config['options'];

        if ($selector !== null) {
            $key = __CLASS__ . '.' . md5($name . $selector . serialize($options));
            $options = !empty($options) ? CJavaScript::encode($options) : '';
            Yii::app()->clientScript->registerScript($key,
