...
(function (e) {
	...
	e.extend(w.prototype, function () {
		...
		function M(t) {
			...
			g = 0,
			b = m.length;
			for (var w = 0, E = m.length; w < E; w++) g = Math.max(g, m[w].length);
			var S = {
				left: 5,
				top: 5
			},
			x = t.contentElement.firstChild;
			x && (x = e(x), S.left = parseInt(x.css("padding-left")) || 0, S.top = parseInt(x.css("padding-top")) || 0),
			v.drawPixelArray(p, m, {
				x: n.background.position.left + n.background.dimensions.width - u - (S.left || 0) - g,
				y: n.background.position.top + n.background.dimensions.height - u - (S.top || 0) - b,
	...
	C.Position = function () {
		...
		function T(t, i, s, o) {
			...
			if (c.stem && c.target != "mouse") {
				...
				H = M + D + .5 * c.stem.width - .5 * A.width,
				B = P > H ? P - H: 0;
				sideOffset = Math.ceil(M + D + .5 * c.stem.width + B + c.stem.offset[k == "horizontal" ? "x": "y"]);
			...
			var R = e.extend({},
			p);
			R = x(R, j, o, l.options.target == "mouse"),
			u.top += R.y,
			u.left += R.x;
			...
			if (t.options.containment) {
				...
				J);
				if (X.overlap < 1) {
					if (W.left < J.position.left || W.left + Q.width > J.position.left + J.dimensions.width) X.horizontal = !1,
					W.left < J.position.left ? V.x = W.left - J.position.left: V.x = W.left + Q.width - (J.position.left + J.dimensions.width);
					if (W.top < J.position.top || W.top + Q.height > J.position.top + J.dimensions.height) X.vertical = !1,
					W.top < J.position.top ? V.y = W.top - J.position.top: V.y = W.top + Q.height - (J.position.top + J.dimensions.height)
				}
			} else X.overlap = 1;
			X.correction = V;
			var G = U[i].bubble,
